...
PROMO_START = datetime(2026, 8, 17)
PROMO_END = datetime(2026, 8, 20, 23, 59, 59)
PROMO_PACKAGE_KEY = "1month"
PROMO_PRICE = 170_080
...
def get_price_display(package_key: str, data: dict) -> str:
    """Return an HTML-formatted price string, with strikethrough
    original price during the promo window for the 1 bulan package."""

    if package_key == PROMO_PACKAGE_KEY and is_promo_active():
        return (
            f"<s>Rp {data['price']:,}</s> ➜ "
            f"<b>Rp {PROMO_PRICE:,}</b>\n"
            f"🎉 Spesial 17–20 Agustus"
        )

    return f"Rp {data['price']:,}"


def get_final_price(package_key: str, data: dict) -> int:
    if package_key == PROMO_PACKAGE_KEY and is_promo_active():
        return PROMO_PRICE
    return data["price"]
...
async def choose_package(
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🥇 STARTER • 1 Bulan | Rp170.080",
...
🥇 <b>STARTER PLAN</b>
📅 1 Bulan
💰 <s>Rp250.000</s> ➜ <b>Rp170.080</b>
🎉 Spesial 17–20 Agustus
...
async def show_payment(
    ...
    price_display = get_price_display(package_key, data)

    payment_text = f"""💳 <b>AKTIVASI MEMBERSHIP</b>
...
async def verify(
    ...
    data = PACKAGE_MAP[package_key]
    final_price = get_final_price(package_key, data)
